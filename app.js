...
${relationship_context}`;
  ...
  var VERSION_HISTORY = {
    "1.63": [
      "ストリーミング表示を追加しました。返事を書かれる端から表示するので、書き終わるまでの無音がなくなります。書き終わる時刻そのものは変わりませんが、長い返事ほど待ち時間が短く感じられるはずです。設定の「アドバンスド」→「ストリーミング出力を使用する」から有効にできます（現在は Gemini のみ。既定はOFFです）。",
